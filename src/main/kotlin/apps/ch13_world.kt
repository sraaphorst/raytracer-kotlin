...
fun main() {
    ...
    val concentricCylinder1 = run {
        val t = Matrix.translate(1, 0, 0) * Matrix.scale(0.8, 1, 0.8)
        val m = Material(Color(1, 1, 0.3), ambient = 0.1,
            diffuse = 0.8, specular = 0.9, shininess = 300.0)
        Cylinder(0, 0.2, false, t, m)
    }

    val concentricCylinder2 = run {
        val t = Matrix.translate(1, 0,0) * Matrix.scale(0.6, 1, 0.6)
        val m = Material(Color(1, 0.9, 0.4), ambient = 0.1,
            diffuse = 0.8, specular = 0.9, shininess = 300.0)
        Cylinder(0, 0.3, false, t, m)
    }

    val concentricCylinder3 = run {
        val t = Matrix.translate(1, 0, 0) * Matrix.scale(0.4, 1, 0.4)
        val m = Material(Color(1, 0.8, 0.5), ambient = 0.1,
            diffuse = 0.8, specular = 0.9, shininess = 300.0)
        Cylinder(0, 0.4, false, t, m)
    }

    val concentricCylinder4 = run {
        val t = Matrix.translate(1, 0, 0) * Matrix.scale(0.2, 1, 0.2)
        val m = Material(Color(1, 0.7, 0.6), ambient = 0.1,
            diffuse = 0.8, specular = 0.9, shininess = 300.0)
        Cylinder(0, 0.5, true, t, m)
    }
    ...
    val world = run {
        ...
        World(listOf(flooring, cylinder,
            concentricCylinder1, concentricCylinder2, concentricCylinder3, concentricCylinder4,
